...
def generate_embeddings(texts: List[str], batch_size: int = 32) -> np.ndarray:
    ...
    """
    model = SentenceTransformer('sentence-transformers/all-mpnet-base-v2') # Changed model name
...
def load_movies_with_embeddings_from_hub() -> List[Movie]:
    ...
    for item in dataset:
        movie = Movie(
            ...
            all_mpnet_base_v2_embedding=item.get('all_mpnet_base_v2_embedding', []) # Changed embedding field name
        )
        # Only include movies that actually have an embedding
        if movie.all_mpnet_base_v2_embedding: # Changed embedding field name
            movies.append(movie)
...
def get_recommendations(
    ...
    if query_is_movie_title:
        ...
        if query_movie and query_movie.all_mpnet_base_v2_embedding: # Changed embedding field name
            query_embedding = np.array(query_movie.all_mpnet_base_v2_embedding) # Changed embedding field name
    ...
    for movie in movies:
        if movie.all_mpnet_base_v2_embedding: # Changed embedding field name
            movie_embedding = np.array(movie.all_mpnet_base_v2_embedding) # Changed embedding field name
